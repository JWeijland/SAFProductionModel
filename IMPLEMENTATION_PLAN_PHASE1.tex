...
\section{Phase 2 Implementation Summary}

Phase 2 is \textbf{completed} - integrates contract pricing into EBIT and NPV calculations. All code changes marked with \texttt{CLAUDE START/END} comments.

\subsection{Files Modified in Phase 2}

\begin{itemize}
    \item \textbf{Modified:} \texttt{src/Agents/Investor.py} - Updated EBIT and NPV calculations (120 lines modified)
    \begin{itemize}
        \item Added \texttt{current\_year} parameter to \texttt{calculate\_ebit()}
        \item Added \texttt{contract\_price} and \texttt{start\_year} parameters to \texttt{calculate\_npv()}
        \item Imported \texttt{year\_for\_tick} utility function
    \end{itemize}
\end{itemize}

\subsection{What Changed in Phase 2}

\textbf{EBIT Calculation (Real-Time Costs):}
\begin{itemize}
    \item[$\checkmark$] Uses \texttt{get\_feedstock\_cost()} to calculate blended costs (85\% contract + 15\% spot)
    \item[$\checkmark$] Contract prices escalate at 3\% annually from initial SRMC
    \item[$\checkmark$] After 20 years, contracts expire and plant uses 100\% spot pricing
    \item[$\checkmark$] Calculates \texttt{effective\_srmc} with blended feedstock costs
    \item[$\checkmark$] Backwards compatible (if \texttt{current\_year} not provided, uses original SRMC)
\end{itemize}

\textbf{NPV Calculation (Investment Forecasting):}
\begin{itemize}
    \item[$\checkmark$] Forecasts contract pricing with 3\% escalation for 20 years
    \item[$\checkmark$] Forecasts spot pricing throughout (using SAF price as proxy: feedstock $\approx$ 40\% of SAF price)
    \item[$\checkmark$] Years 1-20: Uses blended costs (85\% escalated contract + 15\% spot)
    \item[$\checkmark$] Years 21+: Uses 100\% spot pricing (contract expired)
    \item[$\checkmark$] Calculates \texttt{effective\_srmc\_for\_npv} for each forecast year
    \item[$\checkmark$] Backwards compatible (if parameters not provided, uses original SRMC)
\end{itemize}

\subsection{Impact of Phase 2}

\textbf{Before Phase 2 (Phase 1 only):}
\begin{itemize}
    \item Contracts existed but were not used in financial calculations
    \item EBIT used static feedstock price from aggregator
    \item NPV used static SRMC for entire investment horizon
    \item Investment decisions did not account for cost escalation
    \item Result: Model behavior identical to pre-contract implementation
\end{itemize}

\textbf{After Phase 2:}
\begin{itemize}
    \item[$\checkmark$] EBIT reflects actual operating costs with 3\% annual feedstock cost growth
    \item[$\checkmark$] NPV accounts for realistic 20-year cost trajectory
    \item[$\checkmark$] Investment decisions factor in long-term cost escalation
    \item[$\checkmark$] Plants become less profitable over time as feedstock costs rise
    \item[$\checkmark$] After year 20, exposed to full spot market volatility
    \item[$\checkmark$] Result: Model now shows realistic contract economics with escalating costs
\end{itemize}

\subsection{Testing Phase 2}

\begin{itemize}
    \item[$\checkmark$] All 31 Phase 1 tests still pass (backwards compatibility verified)
    \item[$\checkmark$] EBIT calculation tested with mock objects
    \item[$\checkmark$] NPV calculation includes contract forecasting logic
    \item[$\checkmark$] Call sites updated to pass \texttt{current\_year}, \texttt{contract\_price}, \texttt{start\_year}
\end{itemize}

\subsection{Git Status}

\textbf{Branch:} \texttt{feature/feedstock-contracts} \\
\textbf{Commits:} 9 commits (7 Phase 1 + 2 Phase 2) \\
\textbf{Status:} Ready to merge to main when user approves

\textbf{Phase 1 Commits:}
\begin{enumerate}
    \item Create FeedstockContract dataclass
    \item Add contract pricing functions
    \item Add Investor contract methods
    \item Add Aggregator contract tracking
    \item Integrate contracts into Model
    \item Add contract data collection
    \item Add integration tests
\end{enumerate}

\textbf{Phase 2 Commits:}
\begin{enumerate}
    \item Implement contract-aware EBIT and NPV calculations
    \item Fix critical bug: Contract price doubling (SRMC vs feedstock)
    \item Fix escalation: Base feedstock price must increase over time
\end{enumerate}

\section{Critical Fix: Base Feedstock Price Escalation}

\subsection{Problem Discovered}

After Phase 2 implementation, production graphs showed periodic gaps where plants stopped producing unexpectedly. Root cause analysis revealed:

\textbf{Symptom:}
\begin{itemize}
    \item Older plants (built in 2025) would stop producing in 2040
    \item Production would resume in 2054 when contracts expired
    \item Pattern repeated: produce → stop → produce again
\end{itemize}

\textbf{Root Cause:}
\begin{lstlisting}[language=Python, caption=Unrealistic Contract Pricing]
# OLD (BROKEN):
Year 2025: Plant A → contract @ $360 → SRMC = $1,496
Year 2040: Plant A → escalated @ $561 → SRMC = $1,666 (expensive!)
Year 2040: Plant B (NEW) → contract @ $360 → SRMC = $1,496 (cheap!)

# Result: Plant B sets market price at $1,496
#         Plant A cannot compete at $1,666 → STOPS producing
\end{lstlisting}

\textbf{Why This Is Wrong:}
\begin{itemize}
    \item New plants always got contracts at \textbf{fixed} CSV price (\$360-468)
    \item Old plants' costs escalated 3\%/year, new plants stayed cheap
    \item Merit order didn't shift up → old plants became uncompetitive
    \item In reality: feedstock prices rise for \textbf{everyone}, not just old contracts
\end{itemize}

\subsection{Solution Implemented}

Escalate the base feedstock price when creating new contracts:

\begin{lstlisting}[language=Python, caption=Realistic Market Escalation]
# CLAUDE START - Phase 2 FIX
base_feedstock_price = aggregator.feedstock_price  # From CSV: $360
start_year = int(self.model.config["start_year"])
years_since_model_start = current_year - start_year

# Escalate to current market level
escalated_base_price = base_feedstock_price * (
    (1 + escalation_rate) ** years_since_model_start
)

# Use escalated price for new contracts
contract = FeedstockContract(
    initial_contract_price=escalated_base_price,  # NOT base price!
    ...
)
...
\begin{lstlisting}[language=Python, caption=All Plants Competitive]
Year 2025: Plant A → contract @ $360 → SRMC = $1,496
Year 2040: Plant A → escalated @ $561 → SRMC = $1,666
Year 2040: Plant B (NEW) → contract @ $561 → SRMC = $1,666 (SAME!)

# Result: All plants have similar costs
#         Market price rises naturally with inflation
...
\textbf{Before Fix:}
\begin{itemize}
    \item[$\times$] New plants always cheaper than old plants
    \item[$\times$] Merit order didn't shift up over time
    \item[$\times$] Old plants periodically stopped producing (unrealistic)
    \item[$\times$] Production gaps in output graphs
\end{itemize}

\textbf{After Fix:}
\begin{itemize}
    \item[$\checkmark$] All plants have similar SRMC in their start year
    \item[$\checkmark$] Merit order shifts up realistically with market inflation
    \item[$\checkmark$] Market price increases over time
    \item[$\checkmark$] Continuous production (no artificial gaps)
    \item[$\checkmark$] Realistic representation of feedstock market dynamics
\end{itemize}
...
\begin{itemize}
    \item \texttt{src/Agents/Investor.py}:
    \begin{itemize}
        \item \texttt{create\_contract()}: Calculate \texttt{escalated\_base\_price}
        \item \texttt{evaluate\_investment()}: Use escalated price in NPV calculation
    \end{itemize}
    \item \texttt{tests/test\_contract\_integration.py}: Updated test expectations
\end{itemize}

\textbf{Testing:}
\begin{itemize}
    \item[$\checkmark$] All 31 contract tests passing
    \item[$\checkmark$] Verified SRMC consistency across plant vintages
    \item[$\checkmark$] Confirmed merit order shifts up over time
\end{itemize}

\subsection{Final Git Status}

\textbf{Branch:} \texttt{feature/feedstock-contracts} \\
\textbf{Total Commits:} 12 commits
\begin{enumerate}
    \item Phase 1: 8 commits (contract infrastructure)
    \item Phase 2: 2 commits (EBIT/NPV integration)
    \item Bug fixes: 2 commits (SRMC doubling, base price escalation)
\end{enumerate}

\textbf{Tests:} 31/31 passing ✅ \\
\textbf{Status:} Ready for user testing and merge to main
